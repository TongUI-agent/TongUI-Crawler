...
def process_audio_file(sound_file, recognizer, vad_config):
    """ 处理单个音频文件，生成字幕文件 """
    ...
    frames_per_read = int(16000 * 100)  # 100 秒
    ...
    while not is_eof:
        ...
        while not vad.empty():
            segment = Segment(
                start=vad.front.start / 16000,
                duration=len(vad.front.samples) / 16000,
            )
    ...
    # 保存 SRT 字幕文件
    srt_filename = sound_file.with_suffix(".srt")
...
def main():
    ...
    for mp3_file in mp3_files:
        process_audio_file(mp3_file, recognizer, vad_config)
